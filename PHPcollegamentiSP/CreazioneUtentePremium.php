...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $stmt = $conn->prepare("CALL InserisciUtentePremium(?, ?, ?, ?, ?, ?)");
...
if ($stmt->execute()) {
    ...
    $message = "Utente premium inserito con successo";
    ...
    $error = "Errore durante l'inserimento dell'utente premium: " . $stmt->error;
    header("Cache-Control: no-cache, no-store, must-revalidate");
    header("Pragma: no-cache");
    header("Expires: 0");
