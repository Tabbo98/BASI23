...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $stmt = $conn->prepare("CALL InserisciUtenteAmministratore(?, ?)");
    ...
    if ($stmt->execute()) {
        ...
        $message = "Utente amministratore inserito con successo";
        ...
        $error = "Errore durante l'inserimento dell'utente amministratore: " . $stmt->error;
        header("Cache-Control: no-cache, no-store, must-revalidate");
        header("Pragma: no-cache");
        header("Expires: 0");
